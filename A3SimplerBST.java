...
public class A3SimplerBST<K extends Comparable<? super K>, V> {
	...
	// Path length from root to a leaf can be defined as the number of nodes in path - 1
	public int longestPathLength() {
		return longestPathLength(root);
	}

	// helper method for longets path length
	private int longestPathLength(Node<K, V> x) {
		if (x == null) // if reached end
			return -1;
		else if (x.left == null && x.right == null) // if reached leaf
			return 0;
		// keep moving left and right and caluclate the max
		return 1 + Math.max(longestPathLength(x.left), longestPathLength(x.right)); 
	}
	...
	private int leafCount(Node<K, V> x) {
		if (x == null) // if reached end
			return 0;
		else if (x.left == null && x.right == null) // if reached leaf
			return 1;
		// keep moving left and right and caluclate the total leaves in the left and right subtrees
